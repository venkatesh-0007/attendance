...
object PdfExporter {
    ...
    fun generateAndSharePdf(context: Context, attendance: AttendanceResponse) {
        ...
        val intent = Intent(Intent.ACTION_SEND).apply {
            ...
            putExtra(Intent.EXTRA_SUBJECT, "Attendance Report - ${attendance.studentName ?: "Student"}")
    ...
    private fun generatePdfFile(context: Context, attendance: AttendanceResponse): File? {
        try {
            ...
            textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
            canvas.drawText("Student Name: ${attendance.studentName ?: "N/A"}", 50f, 140f, textPaint)

            textPaint.textSize = 12f
            textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.NORMAL)
            textPaint.color = Color.DKGRAY
            canvas.drawText("Roll Number / ID: ${attendance.rollNumber ?: "N/A"}", 50f, 162f, textPaint)
            ...
            textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
            canvas.drawText("Overall Attendance: ${attendance.totalInfo?.totalPercentage ?: "0.0%"}", 50f, 230f, textPaint)

            textPaint.textSize = 12f
            textPaint.typeface = Typeface.create(Typeface.DEFAULT, Typeface.NORMAL)
            textPaint.color = Color.DKGRAY
            canvas.drawText("Total Attended: ${attendance.totalInfo?.totalAttended ?: 0} / Total Held: ${attendance.totalInfo?.totalHeld ?: 0}", 50f, 250f, textPaint)
            ...
            // Table Rows
            startY += 28f
            val subjects = attendance.subjectwiseSummary ?: emptyList()
            var isAltRow = false
            ...
            subjects.forEach { sub ->
                ...
                val nameStr = if (sub.subjectName.length > 32) sub.subjectName.take(30) + ".." else sub.subjectName
            ...
            val pdfFile = File(pdfDir, "Attendance_Report.pdf")
